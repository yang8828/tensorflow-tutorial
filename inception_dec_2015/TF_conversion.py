...
def main():
    ...
    with tf.Session() as sess:
        ...
        sess.run(init)
        for i in range(STEP):
            train_bottlenecks,train_ground_truth = get_random_cached_bottlenecks(sess,n_class,images_lists,BATCH,'training',jpeg_data_tensor,bottleneck_tensor)
            sess.run(train_step,feed_dict={bottleneck_input:train_bottlenecks,ground_truth_input:train_ground_truth})

            if i % 100 == 0 or i + 1 == STEP:
                validation_bottlenecks, validation_ground_truth = get_random_cached_bottlenecks(sess, n_class, images_lists, BATCH, 'validation', jpeg_data_tensor, bottleneck_tensor)
        ...
        test_bottlenecks,test_ground_truth = get_test_bottlenecks(sess,images_lists,n_class,jpeg_data_tensor,bottleneck_tensor)
